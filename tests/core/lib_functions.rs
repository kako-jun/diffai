...
fn test_diffx_core_optimization_path() {
    ...
    let name_change = differences
        .iter()
        .find(|d| matches!(d, DiffResult::Modified(path, _, _) if path.contains("name")));
    let theme_change = differences
        .iter()
        .find(|d| matches!(d, DiffResult::Modified(path, _, _) if path.contains("theme")));
