...
fn test_safetensors_format() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"safetensors": {"version": "1.0", "secure": true}}"#, ".json");
    let file2 = create_temp_file(r#"{"safetensors": {"version": "1.1", "secure": true}}"#, ".json");
...
fn test_numpy_format() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"numpy": {"array": [1, 2, 3], "dtype": "int32"}}"#, ".json");
    let file2 = create_temp_file(r#"{"numpy": {"array": [1, 2, 4], "dtype": "int32"}}"#, ".json");
...
fn test_npz_format() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"npz": {"data1": [1, 2, 3], "data2": [4, 5, 6]}}"#, ".json");
    let file2 = create_temp_file(r#"{"npz": {"data1": [1, 2, 3], "data2": [4, 5, 7]}}"#, ".json");
...
fn test_matlab_format() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"matlab": {"variables": {"result": 0.85}, "complex": true}}"#, ".json");
    let file2 = create_temp_file(r#"{"matlab": {"variables": {"result": 0.90}, "complex": true}}"#, ".json");
