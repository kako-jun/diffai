...
fn test_pytorch_models_comprehensive() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"state_dict": {"fc1.weight": [0.1, 0.2], "fc1.bias": [0.01]}}"#, ".json");
    let file2 = create_temp_file(r#"{"state_dict": {"fc1.weight": [0.15, 0.25], "fc1.bias": [0.02]}}"#, ".json");
...
fn test_safetensors_models_comprehensive() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"tensors": {"layer1.weight": {"shape": [64, 32]}}}"#, ".json");
    let file2 = create_temp_file(r#"{"tensors": {"layer1.weight": {"shape": [64, 64]}}}"#, ".json");
...
fn test_automatic_format_detection() -> Result<(), Box<dyn std::error::Error>> {
    let pretrained = create_temp_file(r#"{"model": {"pretrained": true, "accuracy": 0.85}}"#, ".json");
    let finetuned = create_temp_file(r#"{"model": {"pretrained": false, "accuracy": 0.92}}"#, ".json");
...
fn test_epsilon_tolerance_minor() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--epsilon").arg("1e-6");
    cmd.assert()
        .code(1)
...
fn test_quantization_analysis_epsilon() -> Result<(), Box<dyn std::error::Error>> {
    let fp32 = create_temp_file(r#"{"model": {"precision": "fp32", "weights": [0.123, 0.456]}}"#, ".json");
    let int8 = create_temp_file(r#"{"model": {"precision": "int8", "weights": [0.12, 0.46]}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg(fp32.path()).arg(int8.path()).arg("--epsilon").arg("0.1");
    cmd.assert()
        .code(1)
...
fn test_focus_specific_layers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--path").arg("classifier");
    cmd.assert()
        .code(1)
...
fn test_ignore_metadata() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"timestamp": "2024-01-01", "weights": {"layer1": 0.5}}"#, ".json");
    let file2 = create_temp_file(r#"{"timestamp": "2024-01-02", "weights": {"layer1": 0.6}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--ignore-keys-regex").arg("^timestamp$");
    cmd.assert()
        .code(1)
        .stdout(predicates::str::contains("weights"));
...
fn test_finetuning_analysis() -> Result<(), Box<dyn std::error::Error>> {
    let pretrained = create_temp_file(r#"{"bert": {"encoder": {"attention": {"query": {"weight": [0.001]}}}}, "classifier": {"weight": [0.0]}}"#, ".json");
    let finetuned = create_temp_file(r#"{"bert": {"encoder": {"attention": {"query": {"weight": [0.0023]}}}}, "classifier": {"weight": [0.0145]}}"#, ".json");
...
fn test_quantization_impact_assessment() -> Result<(), Box<dyn std::error::Error>> {
    let fp32 = create_temp_file(r#"{"conv1": {"weight": {"mean": 0.0045, "std": 0.2341}}}"#, ".json");
    let int8 = create_temp_file(r#"{"conv1": {"weight": {"mean": 0.0043, "std": 0.2298}}}"#, ".json");
...
fn test_training_progress_tracking() -> Result<(), Box<dyn std::error::Error>> {
    let epoch10 = create_temp_file(r#"{"layers": {"0": {"weight": {"mean": -0.0012, "std": 1.2341}}, "1": {"bias": {"mean": 0.1234, "std": 0.4567}}}}"#, ".json");
    let epoch50 = create_temp_file(r#"{"layers": {"0": {"weight": {"mean": 0.0034, "std": 0.8907}}, "1": {"bias": {"mean": 0.0567, "std": 0.3210}}}}"#, ".json");
...
fn test_architecture_comparison() -> Result<(), Box<dyn std::error::Error>> {
    let resnet = create_temp_file(r#"{"features": {"conv1": {"weight": {"shape": [64, 3, 7, 7]}}, "layer4": {"2": {"downsample": {"0": {"weight": {"shape": [2048, 1024, 1, 1]}}}}}}}"#, ".json");
    let efficientnet = create_temp_file(r#"{"features": {"conv1": {"weight": {"shape": [32, 3, 3, 3]}}, "mbconv": {"expand_conv": {"weight": {"shape": [96, 32, 1, 1]}}}}}"#, ".json");
...
fn test_focus_analysis_specific_parts() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"tensor": {"classifier": {"weight": [0.1, 0.2]}}}"#, ".json");
    let file2 = create_temp_file(r#"{"tensor": {"classifier": {"weight": [0.15, 0.25]}}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--path").arg("tensor.classifier");
    cmd.assert()
        .code(1)
        .stdout(predicates::str::contains("classifier"));
...
fn test_higher_epsilon_faster_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--epsilon").arg("1e-3");
    cmd.assert()
        .code(1)
...
fn test_architecture_differences_only() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"architecture": {"type": "transformer", "layers": 12}}"#, ".json");
    let file2 = create_temp_file(r#"{"architecture": {"type": "transformer", "layers": 24}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg(file1.path()).arg(file2.path()).arg("--architecture-comparison");
    cmd.assert()
        .code(1)
...
fn test_cicd_compare_models() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let candidate = create_temp_file(r#"{"model": {"type": "candidate", "accuracy": 0.88}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg(baseline.path()).arg(candidate.path()).arg("--output").arg("json");
    cmd.assert()
        .code(1)
        .stdout(predicates::str::is_match(r#"\[.*\]"#).unwrap());
...
fn test_explicit_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file2 = create_temp_file(r#"{"safetensors": {"format": "explicit", "version": 2}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg("--format").arg("safetensors").arg(file1.path()).arg(file2.path());
    cmd.assert()
        .code(1)
...
fn test_memory_optimization_epsilon() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file2 = create_temp_file(r#"{"large": {"tensor": [0.0015, 0.0025, 0.0035]}}"#, ".json");
    
    let mut cmd = diffai_cmd();
    cmd.arg("--epsilon").arg("1e-3").arg(file1.path()).arg(file2.path());
    cmd.assert()
        .code(1)
...
fn test_memory_optimization_path() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = diffai_cmd();
    cmd.arg("--path").arg("tensor.classifier").arg(file1.path()).arg(file2.path());
    cmd.assert()
        .code(1)
        .stdout(predicates::str::contains("classifier"));
...
fn test_comprehensive_analysis_automatic() -> Result<(), Box<dyn std::error::Error>> {
    let epoch10 = create_temp_file(r#"{"checkpoint": {"epoch": 10, "loss": 0.5, "accuracy": 0.8}}"#, ".json");
    let epoch20 = create_temp_file(r#"{"checkpoint": {"epoch": 20, "loss": 0.3, "accuracy": 0.9}}"#, ".json");
...
fn test_experimental_comparison_automatic() -> Result<(), Box<dyn std::error::Error>> {
    let baseline = create_temp_file(r#"{"experiment": {"type": "baseline", "performance": 0.85}}"#, ".json");
    let experiment = create_temp_file(r#"{"experiment": {"type": "enhanced", "performance": 0.92}}"#, ".json");
