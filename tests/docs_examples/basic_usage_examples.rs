...
fn test_basic_comprehensive_analysis() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"model": {"layers": 2, "params": 1000}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"model": {"layers": 3, "params": 1500}}"#, ".safetensors");
...
fn test_json_output() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"tensor": {"mean": 0.5, "std": 0.1}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"tensor": {"mean": 0.6, "std": 0.2}}"#, ".safetensors");
...
fn test_yaml_output() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"weights": {"layer1": 0.5}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"weights": {"layer1": 0.7}}"#, ".safetensors");
...
fn test_recursive_directory_comparison() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"config": {"version": "1.0"}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"config": {"version": "2.0"}}"#, ".safetensors");
...
fn test_recursive_with_format() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"model": {"type": "safetensors"}}"#, ".safetensors");
...
fn test_pytorch_model_comparison() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"state_dict": {"layer1.weight": [0.1, 0.2]}}"#, ".pt");
    let file2 = create_temp_file(
...
fn test_training_checkpoint_comparison() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"epoch": 1, "loss": 0.8, "accuracy": 0.6}"#, ".pt");
    let file2 = create_temp_file(r#"{"epoch": 10, "loss": 0.3, "accuracy": 0.9}"#, ".pt");
...
fn test_baseline_vs_improved() -> Result<(), Box<dyn std::error::Error>> {
    let baseline = create_temp_file(r#"{"performance": 0.85, "params": 1000000}"#, ".pt");
    let improved = create_temp_file(r#"{"performance": 0.92, "params": 1200000}"#, ".pt");
...
fn test_safetensors_comprehensive() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"tensors": {"fc1.bias": {"shape": [64]}}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"tensors": {"fc1.bias": {"shape": [128]}}}"#, ".safetensors");
...
fn test_deployment_validation() -> Result<(), Box<dyn std::error::Error>> {
    let baseline = create_temp_file(r#"{"deployment": {"ready": true, "risk": "low"}}"#, ".safetensors");
...
fn test_compressed_numpy_archives() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"dataset": {"train": 1000, "test": 200}}"#, ".npz");
    let file2 = create_temp_file(r#"{"dataset": {"train": 1200, "test": 250}}"#, ".npz");
...
fn test_experiment_comparison() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"experiment": {"id": "v1", "accuracy": 0.85}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"experiment": {"id": "v2", "accuracy": 0.90}}"#, ".safetensors");
...
fn test_checkpoint_learning_analysis() -> Result<(), Box<dyn std::error::Error>> {
    let file1 = create_temp_file(r#"{"checkpoint": {"epoch": 10, "loss": 0.5}}"#, ".safetensors");
    let file2 = create_temp_file(r#"{"checkpoint": {"epoch": 20, "loss": 0.3}}"#, ".safetensors");
