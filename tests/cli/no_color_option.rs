...
fn create_test_json1() -> Result<NamedTempFile, Box<dyn std::error::Error>> {
    ...
    writeln!(temp_file, r#"{{"name": "test", "value": 123, "enabled": true}}"#)?;
    Ok(temp_file)
}

fn create_test_json2() -> Result<NamedTempFile, Box<dyn std::error::Error>> {
    let mut temp_file = NamedTempFile::new()?;
    writeln!(temp_file, r#"{{"name": "test", "value": 456, "enabled": false}}"#)?;
    Ok(temp_file)
}
...
fn test_diffai_no_color_option_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !stdout.trim().is_empty() {
        assert!(stdout.contains("value") || stdout.contains("123") || stdout.contains("456") || stdout.contains("~"),
    ...
    for format in &formats {
        let output = Command::new("cargo")
            .args(["run", "--bin", "diffai", "--", 
                   test_file1.path().to_str().unwrap(),
                   test_file2.path().to_str().unwrap(),
                   "--no-color", "--output", format])
            .current_dir(env!("CARGO_MANIFEST_DIR"))
            .output()
            .expect(&format!("Failed to execute diffai with --no-color and --output {}", format));
