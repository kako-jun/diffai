...
cargo search diffai-core
npm view diffai-js version
pip show diffai-python

# GitHubリリースページも確認
# https://github.com/kako-jun/diffai/releases
```
...
# diffai-cli/src/main.rs の Args 構造体をチェック

# 現在の実装済みオプション（v0.5.1基準）：
# - input1, input2: 入力ファイル/ディレクトリ
# - --format: 入力フォーマット (json/yaml/toml/ini/xml/csv)
# - --output: 出力フォーマット (cli/json/yaml/unified)
# - --recursive: ディレクトリ再帰比較
# - --path: 特定パスのフィルタ
# - --ignore-keys-regex: キー除外正規表現
# - --epsilon: 浮動小数点許容誤差
# - --array-id-key: 配列要素識別キー
# - --context: unified diff形式のコンテキスト行数
# - --ignore-whitespace: 空白文字無視
...
# npmラッパーの対応確認
cd diffai-npm && node examples.js
...
# 2. crates.ioの確認
cargo search diffai-core
cargo search diffai-cli
# または直接ブラウザで確認:
# https://crates.io/crates/diffai-core
# https://crates.io/crates/diffai-cli

# 3. PyPIの確認
pip index versions diffai-python
# または直接ブラウザで確認:
# https://pypi.org/project/diffai-python/

# 4. npmの確認
npm view diffai-js versions --json
# または直接ブラウザで確認:
...
# crates.io: cargo yank --vers <version> diffai-core
# npm: npm unpublish diffai-js@<version> (24時間以内のみ可能)
# PyPI: サポートに連絡が必要
```
...
- 例: v0.4.2が成功、v0.5.0が失敗、v0.5.1が成功の場合 → v0.5.1には v0.4.2からv0.5.1までの全変更を記載
...
### ソースコードベースでの機能確認
- **注意**: システムにインストール済みの古いdiffaiバイナリで`--help`を確認してはいけない
- 必ずソースコード（diffai-cli/src/main.rs）の Args 構造体から最新オプションを確認
...
#### 修正済み項目（v0.5.1+）
- **diffai-python/src/diffai/__init__.py**: `__version__`を動的取得に変更
...
git push --delete origin v0.6.0-test
...
```bash
# 1. リバート用ブランチ準備
git checkout -b emergency-revert-v0.6.0

# 2. 前回成功バージョンの記録
echo "Last successful release: $(git tag --sort=-version:refname | head -2 | tail -1)"

# 3. ロールバック手順の確認
./scripts/cleanup-failed-release.sh --dry-run v0.6.0
```
...
### ドキュメント更新の同期
```bash
# 1. CHANGELOG.mdの更新（リリースノート充実化と同時実施）
# 2. README.mdの新機能説明
# 3. ドキュメントサイトの更新準備
# 4. リリースノートのテンプレート準備
```
...
### Rustに新オプション追加時
1. diffai-cli/src/main.rs でオプション追加
2. diffai-npm/lib.js でオプション対応追加
3. diffai-python/src/diffai/diffai.py でオプション対応追加
4. 各パッケージのREADME更新
5. 各パッケージのexamples更新
...
### crates.ioのみ緊急修正
```bash
cd diffai-core && cargo publish
cd ../diffai-cli && cargo publish
```
