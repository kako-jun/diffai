...
// diffx-core統合: 基本機能を再利用してコード重複を削減
pub use diffx_core::{
    value_type_name,
    ...
    format_diff_output,
    diff as base_diff,
    DiffOptions as BaseDiffOptions,
    OutputFormat as BaseOutputFormat,
};
...
// 古いdiff実装関数は削除 - diffx-coreの機能を活用してコード重複を大幅削減
...
fn analyze_batch_normalization_analysis(
    old_model: &Value,
    new_model: &Value,
    results: &mut Vec<DiffResult>,
) {
...
fn analyze_regularization_impact(
    old_model: &Value,
    new_model: &Value,
    results: &mut Vec<DiffResult>,
) {
...
fn analyze_activation_pattern_analysis(
    old_model: &Value,
    new_model: &Value,
    results: &mut Vec<DiffResult>,
) {
...
fn analyze_weight_distribution_analysis(
    old_model: &Value,
    new_model: &Value,
    results: &mut Vec<DiffResult>,
) {
...
fn analyze_model_complexity_assessment(
    old_model: &Value,
    new_model: &Value,
    results: &mut Vec<DiffResult>,
) {
...
/// これらの関数はdiffx-coreから再エクスポートされているため削除
// pub use diffx_core::{value_type_name, estimate_memory_usage, would_exceed_memory_limit};
...
pub fn format_diff_results(results: &[DiffResult], format: OutputFormat) -> Result<String> {
    // diffx-coreのDiffResultに変換してからフォーマット
    ...
    if !base_results.is_empty() {
        ...
        if !ml_results.is_empty() {
            ...
            // ML専用型の簡単なフォーマット（再帰回避）
            let mut ml_formatted = String::new();
            for result in &ml_results {
                match format {
                    OutputFormat::Json => {
                        ml_formatted.push_str(&serde_json::to_string_pretty(result)?);
                        ml_formatted.push('\n');
                    }
                    OutputFormat::Yaml => {
                        ml_formatted.push_str(&serde_yaml::to_string(result)?);
                    }
                    OutputFormat::Diffai => {
                        ml_formatted.push_str(&serde_json::to_string(result)?);
                        ml_formatted.push('\n');
                    }
                }
            }
            output.push_str(&ml_formatted);
            Ok(output)
        } else {
            Ok(formatted)
        }
