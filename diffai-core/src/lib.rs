...
fn should_analyze_ml_features(old: &Value, new: &Value, _opts: &DiffOptions) -> bool {
    ...
    if let (Value::Object(old_obj), Value::Object(new_obj)) = (old, new) {
        // モデル関連のキーが含まれている場合
        let ml_keys = ["state_dict", "model", "optimizer", "scheduler", "epoch", "loss", "accuracy"];
        for key in &ml_keys {
...
/// Format output to string using diffx-core base functions - FOR INTERNAL USE ONLY  
pub fn format_output<T: Serialize>(results: &[T], format: OutputFormat) -> Result<String> {
    // 基本的なシリアライゼーション（diffx-coreのロジックを参考）
    match format {
...
pub fn format_diff_results(results: &[DiffResult], format: OutputFormat) -> Result<String> {
    ...
    if !base_results.is_empty() {
        ...
        if !ml_results.is_empty() {
            ...
            let ml_formatted = format_output(&ml_results, format)?;
