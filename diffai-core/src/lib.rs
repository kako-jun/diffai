                ...
                Ok(mut file_results) => {
                    ...
                    for result in &mut file_results {
                        match result {
                            ...
                            DiffResult::TensorShapeChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::TensorDataChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::ModelArchitectureChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::WeightSignificantChange(path, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::ActivationFunctionChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::LearningRateChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::OptimizerChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::LossChange(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::AccuracyChange(path, _, _) => *path = format!("{}/{}", rel_path, path),
                            DiffResult::ModelVersionChanged(path, _, _) => *path = format!("{}/{}", rel_path, path),
                        }
                    }
...
fn check_ml_number_changes(
    ...
    if diffai_opts.learning_rate_tracking.unwrap_or(false) && path.contains("learning_rate") {
        results.push(DiffResult::LearningRateChanged(path.to_string(), old_val, new_val));
        return;
    }
    
    // Check for loss changes
    if diffai_opts.loss_tracking.unwrap_or(false) && (path.contains("loss") || path.contains("cost")) {
        results.push(DiffResult::LossChange(path.to_string(), old_val, new_val));
        return;
    }
    
    // Check for accuracy changes
    if diffai_opts.accuracy_tracking.unwrap_or(false) && (path.contains("accuracy") || path.contains("acc")) {
        results.push(DiffResult::AccuracyChange(path.to_string(), old_val, new_val));
        return;
    }
    ...
    if let Some(threshold) = diffai_opts.weight_threshold {
        if change_magnitude > threshold && (path.contains("weight") || path.contains("bias")) {
            results.push(DiffResult::WeightSignificantChange(path.to_string(), change_magnitude));
            return;
        }
    }
    
    // Default to regular modification
    results.push(DiffResult::TensorDataChanged(path.to_string(), old_val, new_val));
}
...
pub fn parse_xml(content: &str) -> Result<Value> {
    ...
    Ok(Value::String(format!("XML parsing not fully implemented: {}", content.len())))
}
...
pub fn parse_pytorch_model(file_path: &Path) -> Result<Value> {
    ...
    let mut result = serde_json::Map::new();
    result.insert("model_type".to_string(), Value::String("pytorch".to_string()));
...
pub fn parse_safetensors_model(file_path: &Path) -> Result<Value> {
    ...
    for tensor_name in safetensors.names() {
        ...
        tensor_info.insert("shape".to_string(), 
            Value::Array(tensor_view.shape().iter().map(|&s| Value::Number(s.into())).collect()));
        tensor_info.insert("dtype".to_string(), 
            Value::String(format!("{:?}", tensor_view.dtype())));
        
        tensors.insert(tensor_name.to_string(), Value::Object(tensor_info));
    }
    
    result.insert("model_type".to_string(), Value::String("safetensors".to_string()));
...
pub fn parse_numpy_file(path: &Path) -> Result<Value> {
    ...
    result.insert("file_path".to_string(), Value::String(path.to_string_lossy().to_string()));
...
pub fn parse_matlab_file(path: &Path) -> Result<Value> {
    ...
    result.insert("model_type".to_string(), Value::String("matlab".to_string()));
    result.insert("file_path".to_string(), Value::String(path.to_string_lossy().to_string()));
