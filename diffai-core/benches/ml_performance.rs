...
fn benchmark_ml_model_comparison(c: &mut Criterion) {
    ...
    let test_model_paths = vec![
        ("simple_base", "tests/fixtures/ml_models/simple_base.safetensors"),
        ("simple_modified", "tests/fixtures/ml_models/simple_modified.safetensors"),
        ("small_model", "tests/fixtures/ml_models/small_model.safetensors"),
    ];
    ...
    for (size_name, path) in test_model_paths {
        if Path::new(path).exists() {
            group.bench_function(&format!("parse_model_{}", size_name), |b| {
                b.iter(|| {
                    black_box(parse_safetensors_model(black_box(Path::new(path))))
                })
            });
        }
    }
