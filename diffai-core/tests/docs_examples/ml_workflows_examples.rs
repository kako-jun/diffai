...
fn test_core_finetuning_comparison() {
    let v1 =
        parse_json(r#"{"model": {"pretrained": true, "weights": {"classifier": [0.0, 0.0]}}}"#);
    let v2 =
        parse_json(r#"{"model": {"pretrained": false, "weights": {"classifier": [0.8, 0.9]}}}"#);
...
fn test_core_workflow_comparison() {
    ...
    let has_workflow_diff = results
        .iter()
        .any(|r| format!("{:?}", r).contains("workflow"));
