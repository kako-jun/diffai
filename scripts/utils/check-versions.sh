...
# Version consistency checker for diffai packages
...
echo "Checking version consistency across diffai packages..."
...
echo "├─────────────────┼─────────────┤"
printf "│ %-15s │ %-11s │\n" "diffai-core" "$CORE_VERSION"
printf "│ %-15s │ %-11s │\n" "diffai-cli" "$CLI_VERSION"
printf "│ %-15s │ %-11s │\n" "diffai-js" "$NPM_VERSION"
printf "│ %-15s │ %-11s │\n" "diffai-python" "$PYTHON_VERSION"
echo "└─────────────────┴─────────────┘"
echo ""
...
if [ "$CORE_VERSION" != "$CLI_VERSION" ]; then
    ...
    echo "   diffai-core: $CORE_VERSION"
    echo "   diffai-cli:  $CLI_VERSION"
...
if [ "$NPM_VERSION" = "not found" ]; then
    warning "npm package not found (diffai-npm/package.json missing)"
...
if [ "$PYTHON_VERSION" = "not found" ]; then
    warning "Python package not found (diffai-python/pyproject.toml missing)"
